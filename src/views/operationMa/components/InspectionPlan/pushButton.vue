<template xmlns:el-col="http://www.w3.org/1999/html">
  <div>
    <div class="s">
      <el-button type="primary" size="small" class="btn-middle" @click="openAddWin">新增任务</el-button>
    </div>

    <!-- 新增运维任务弹出层 -->
    <el-dialog title="新增运维任务" :modal-append-to-body="false" :close-on-click-modal="false" :visible.sync="addWin"
               class="yw" width="1150px" top="18vh" style="left:80px">
      <div>
        <div class="form-item">
          <div class="form-item-title"><span class="line"></span>基本信息</div>
          <div class="form-item-content">
            <el-form ref="inspectTaskForm" :model="inspectTaskForm" :rules="rules" label-width="120px" size="small"
                     class="xy">
              <el-row>
                <el-col :span="8">
                  <el-form-item label="任务名称" prop="taskName">
                    <el-input class="kz" size="mini" v-model="inspectTaskForm.taskName"
                              placeholder="请输入任务名称"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="任务类型" prop="taskType">
                    <el-select v-model="inspectTaskForm.taskType" class="kz" size="mini" placeholder="请选择" clearable>
                      <el-option v-for="item in devOpsTaskTypes"
                                 :key="item.value"
                                 :label="item.name"
                                 :value="item.value">
                      </el-option>
                ...
                <el-col :span="8">
                  <el-form-item label="审核人" prop="checkManId">
                    <el-select v-model="nodeUsersValue.name" size="mini" class="kz" placeholder="请选择"
                               clearable ref="nodeUSelect">
                      <el-option :value="nodeUsersValue" :key="nodeUsersValue.value" :label="nodeUsersValue.name"
                        ...
                        <el-tree
                          ...
                          <span slot-scope="{ node, data }">
                                   <span>
                                    <i v-if="node.expanded && node.childNodes.length==0"
                                       class="el-icon-document-remove"></i>
                                    <i v-if="node.expanded && node.childNodes.length>0"
                                       class="el-icon-folder-opened"></i>
                                    <i v-else-if="data.children!=null && data.children.length==0"
                                       class="el-icon-document-remove"></i>
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item label="组织机构" prop="orgId">
                    <el-select v-model="nodeValue.value" size="mini" class="kz" placeholder="请选择组织机构树"
                               :disabled="disabledState"
                               clearable ref="nodeSelect">
                      <el-option :value="nodeValue.value" :key="nodeValue.value" :label="nodeValue.name"
                                 style="width: auto;height:auto;overflow: auto;background-color:#fff">
                        <el-tree
                          :props="props"
                          :data="orgsTree"
                ...
                <el-col :span="8">
                  <el-form-item label="周期" prop="period">
                    <el-select class="kz" v-model="inspectTaskForm.period" placeholder="请选择" size="mini">
                      <el-option v-for="item in cycleTimes"
                                 :key="item.value"
                                 :label="item.name"
                ...
                <el-col :span="8">
                  <el-form-item label="频次" prop="freq">
                    <el-input class="kz" size="mini" v-model="inspectTaskForm.freq" placeholder="请输入频次"/>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item label="开始执行时间" prop="startDate">
                    <el-date-picker class="kz" v-model="inspectTaskForm.startDate" format="yyyy-MM-dd HH:mm"
                ...
                <el-col :span="8">
                  <el-form-item label="结束执行时间" prop="endDate">
                    <el-date-picker class="kz" v-model="inspectTaskForm.endDate" format="yyyy-MM-dd HH:mm"
                                    type="datetime" size="mini"
                                    placeholder="选择日期"></el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="间隔时间" prop="intervalDays">
                    <el-input class="kz" size="mini" v-model="inspectTaskForm.intervalDays"
        ...
        <div class="form-item">
          <div class="form-item-title">
            ...
            <el-select class="kz" v-model="devopsSchemesV.name" size="mini"
                       placeholder="请选择" @change="checkClick">
              <el-option v-for="item in devopsSchemes"
                         :key="item.id"
                         :label="item.name"
                         :value="item">
              </el-option>
                ...
                <el-col :span="24">
                  <el-form-item label="预案名称：" prop="inspectContentId">
                    <a href="javascript:void(0)" type="primary">
                      <span v-text="devopsSchemesV.name" @click="openDetailWin"></span></a>
                  </el-form-item>
                </el-col>
              </el-row>
              <br/>
        ...
        <div class="form-item">
          <div class="form-item-title">
            ...
            <el-select size="mini" v-model="devicesTargetValue.name" class="kz" placeholder="请选择"
                       @change="changeSelect">
              <el-option v-for="item in devopsDevicesTarget"
                         :key="item.value"
                         :label="item.name"
                         :value="item.value">
              </el-option>
                ...
                <div class="search-wrap">
                  <el-form :inline="true" class="demo-form-inline">
                    <el-form-item label="组织机构名称">
                      <el-input size="mini" placeholder="请输入组织机构名称" class="inputWidth" v-model="orgName"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item>
                      <el-button size="mini" type="primary" class="searchColor" @click="queryBtnClick">查询</el-button>
                    </el-form-item>
                  </el-form>
                </div>
                <div class="item-sel">
                  <div class="left">
                    ...
                    <div class="cont">
                      ...
                      >
                        ...
                        <el-table-column :show-overflow-tooltip="true" label="所属区域"
                                         prop="regionFullName"/>
                        <el-table-column :show-overflow-tooltip="true" label="详细地址" prop="addresDesc"/>
                      </el-table>
                    </div>
                    <!-- 分页 -->
                    <div class="block">
                ...
                <div class="search-wrap">
                  <el-form :inline="true" class="demo-form-inline">
                    <el-form-item label="站点名称">
                      <el-input size="mini" placeholder="请输入站点名称" class="inputWidth" v-model="orgName"/>
                    </el-form-item>

                    <el-form-item label="站点类型">
                      <el-select size="mini" v-model="siteValue" placeholder="全部" class="selectWidth" clearable>
                        <el-option v-for="item in siteTypes"
                                   :key="item.value"
                                   :value="item"
                                   :label="item.name"/>
                      </el-select>
                ...
                <div class="search-wrap">
                  <el-form :inline="true" class="demo-form-inline">
                    <el-form-item label="组织机构">
                      <el-select v-model="nodeData.value" size="mini" class="kz" placeholder="请选择组织机构树"
                                 :disabled="disabledState"
                                 clearable ref="nodeCSelect">
                        <el-option :value="nodeData.value" :key="nodeData.value" :label="nodeData.name"
                                   style="width: auto;height:auto;overflow: auto;background-color:#fff">
                          <el-tree
                            :props="props"
                            :data="orgsTree"
                    ...
                    <el-form-item label="站点">
                      <el-select size="mini" v-model="chargeDevicesValue" placeholder="全部" clearable
                                 class="selectWidth">
                        <el-option v-for="item in chargeDevices"
                                   :value="item"
                                   :key="item.id"
                                   :label="item.name"/>
                      </el-select>
      ...
      <div class="dialog-footer">
        <el-button type="primary" class="btn-long" size="small" @click="submitAdd('inspectTaskForm')">确认</el-button>
        <el-button size="small" class="btn-long" @click="dialogFormVisible = false">取消</el-button>
      </div>
    </el-dialog>
    <div>
      <!-- 弹出层 查看预案 -->
      <el-dialog title="查看预案" :modal-append-to-body="false" :visible.sync="detailWin" class="yw" width="900px">
        <div>
          <div class="form-item">
            <div class="form-item-title">
          ...
          <div class="form-item">
            ...
            <div class="case-content">
              <el-tree :data="cashTreeData" :props="props" node-key="id"
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          checkManId: [{required: true, message: '请选择审核人', trigger: 'blur'}],
          ...
          inspectContentId: [{required: true, message: '请选择运维内容', trigger: 'blur'}],
          intervalDays: [{required: true, message: '根据频次和执行时间长度进行计算填写', trigger: 'blur'}],
          startDate: [{required: true, message: '请选取开始时间', trigger: 'blur'}],
          endDate: [{required: true, message: '请选取结束时间', trigger: 'blur'}],
        },
        // 显示不同的页面
        tabIsShow: 1,
        // 运维任务
    ...
    methods: {
      ...
      queryBtnClick() {
        ...
        if (this.tabIsShow === 2) {
          console.log(this.regionValue[this.regionValue.length - 1], this.siteValue, this.orgName)
        ...
        if (this.tabIsShow === 3) {
          console.log(this.chargeDevicesValue.staId)
          this.params.staId = this.chargeDevicesValue.staId
          console.log(this.params.staId)
          this.queryOperationQueryDevPage(this.params)
        }
      },
      // 获取组织结构表格数据
