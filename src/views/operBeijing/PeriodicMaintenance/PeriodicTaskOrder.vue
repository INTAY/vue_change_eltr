...
import fillBar from '../components/PeriodicTaskOrder/fillBar';
import planOrder from '../components/PeriodicTaskOrder/planOrder';
...
export default {
    ...
    data() {
        //派发日期
        let showDispatchTime = (row, column,index) => {
            return row.dispatch_time.slice(0,16)
        };
        // 当前周期结束时间
        let showCircelEnd = (row, column,index) => {
            return row.circel_end.slice(0,16)
        };
        // 当前周期开始时间
        let showCircleStart = (row, column,index) => {
            return row.circle_start.slice(0,16)
        };
        return {
            regTreeData: [], //区域数据
            ISDATA: null,
            ...
            fieldList: [
                ...
                { name: '当前周期巡视', value: 'tour_state' },
                { name: '当前周期（天）', value: 'peroid' },
                { name: '完成度', value: 'percent' },
                { name: '派发日期', value: 'dispatch_time', formatter: showDispatchTime },               
                { name: '任务编号', value: 'source_id' },
                { name: '当前周期开始时间', value: 'circle_start', formatter: showCircleStart },
                { name: '当前周期结束时间', value: 'circel_end', formatter: showCircelEnd },
