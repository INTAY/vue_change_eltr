       ...
       <el-form ref="form" :model="zoneForm" label-width="110px" size="small" :rules="rules">
        <div style="border: 1px solid #F2F2F2;background: #F8F8F8;padding: 14px 0px;">
               <div style="padding-left: 20px;margin-bottom: 10px;">
               	...
               	<el-checkbox-group v-model="checkList_">
                ...
                <div class="yi">
                    ...
                    <el-form-item label="站点名称">
                        <el-select size="mini" filterable class="inputWidth" v-model="stationForm.stationId" clearable>
                            <el-option v-for="item in staMap" :key="item.id" :value="item.name" :label="item.name"> </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item>
...
import { addTasks, queryStationAndStakeNum, getListByInspId, queryList,getDetails } from '@/api/operation/planTasks';
import { getListByInspType,queryOperationBjStake,reDispatch } from '@/api/operation/flawAfault';
...
export default {
    data() {
        ...
        return {
            ...
            obj1:{current:1, size:15, total:0,},
            obj2:{current:1, size:15, total:0,},
            pageData: {
                total: 0,
                current: 1,
                size: 0
            ...
            zoneForm: {deadline:'',inspectReason:''},
            rules: {
    ...
    methods: {
    	...
    	handleCheckedCitiesChange(){
    		this.checkLists = this.checkList_.map(item=>item.inspType);
    		this.modelName = this.checkList_.map(item=>item.id);
    	},
    	//关闭站框
    	siteClear(){
        ...
        alreadySelectMap(val) {
    		if (val.length>1) {
    			...
    			this.$message.warning('巡视对象为桩只能修改一个站');
    			return false
    		}else{
...
//          this.stationData = this.unique(arr)
        ...
        getData(id) {
            getDetails(id).then((res) => {
                if (res.code === '000000') {
                	this.imgDatas = res.data.docUrls;
                	let fileList = [];
		            if (!!this.imgDatas) {
		                if (!!this.imgDatas.length) {
		                    for (let i = 0; i < this.imgDatas.length; i++) {
		                        fileList.push({ url: this.imgDatas[i] });
		                    }
		                }
		            }
		            let aaa = res.data.inspectContentDetVos
		            let bbb = []
		            for(let i = 0;i < this.modelList.length; i ++){
		            	for(let j = 0;j < aaa.length; j ++){
			            	if(aaa[j].id == this.modelList[i].id){
			            		bbb.push(this.modelList[i])
			            	}
			            }
		            }
		            this.checkList_ = bbb;
                    ...
                    this.stationData = [
                        {
                            ...
                            staId: res.data.staId,
                            regId: res.data.regId,
                            regName: res.data.regName,
                            stationName: res.data.staName,
			...
			this.$refs.form.validate((valid) => {
	          if (valid) {
	        	...
	        	if (this.stationData && this.stationData.length > 0) {
	        		this.stationData.forEach((item) => {
	                    inspectObjs_.push({
	                        ...
	                        totalStakeNum: item.totalStakeNum,
                            zlStakeNum: item.zlStakeNum,
                            jlStakeNum: item.jlStakeNum,
	                    });
	                });
	        	}else{
	        		inspectObjs_ = []
	        	}
	        	if (this.stationData2&&this.stationData2.length > 0) {
	        	...
	        	if (1) {
		            ...
		            reDispatch(ids_,params)
		                .then((res) => {
		                	if(res.code=="000000"){
			                    this.$message({
			                        message: '重新派发成功,原任务作废',
			                        type: 'success'
			                    });
