                ...
                <div v-for="(item, index) in siteItemMessage" :key="index">
                    ...
                    <section class="toggle-item-content" v-if="item.displate">
                        <el-form ref="form" :model="item" label-width="150px" class="base-info">
                            ...
                            <!-- 问题类型和问题描述只存在于有问题的时候 -->
                            <el-col :span="16" v-if="!item.isOk">
                                <el-form-item label="问题类型：">
                                    <div class="sta-question">占问题</div>
                                    <div v-for="(items,index) in item.qs" :key="index" style="display: inline">
                                        <span>{{ items.name }}</span>
                                        <!-- 判断下标是最后一个就不显示这个竖线 -->
                                        <el-divider v-if="index != item.qs.length-1" direction="vertical"></el-divider>
                                    </div>
                                </el-form-item>
                            </el-col>
                            <!-- 第三行 -->
                            <el-col :span="24" v-if="!item.isOk">
...
export default {
    data() {
        return {
            ...
            tableData: [],
            siteItemMessage: [
                { name: '查看充电站场地么么么么么么么么么', person: '1', displate: false, isOk: false, qs: [{name: '失败'},{name: '失败1'}] },
                { name: '查看充电站场地么么么么么么么么么', person: '1', displate: false, isOk: true }
            ],
            checkAll: false,
            checkedCities: ['上海', '北京'],
            cities: cityOptions,
