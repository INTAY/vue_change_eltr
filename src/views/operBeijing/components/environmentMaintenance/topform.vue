        ...
        <el-form :inline="true" :model="formInline">
            <div>
                ...
                <el-form-item label="任务来源">
                    <el-select size="mini" v-model="formInline.taskSource" placeholder="全部" class="xzk" clearable>
                        <el-option v-for="item in taskSourceList" :key="item.value" :label="item.name" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>

                <el-form-item label="任务状态">
                    <el-select size="mini" v-model="formInline.state" placeholder="全部" class="xzk" clearable>
                        <el-option v-for="item in state" :key="item.value" :label="item.name" :value="item.value" clearable></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-button
                    ...
                    @click="formInline = { taskType: 7, sortColumn: 'created_time', order: 'desc' }"
                    >清空</el-button
                >
            </div>
        </el-form>
...
export default {
    ...
    components: { treeSelect, usersTree },
    created() {
        // 获取任务状态字典
    ...
    methods: {
        ...
        onSubmit() {
            ...
            //
            if (!!this.formInline.getTimes) {
                this.formInline.actualEndTimeFrom = this.formInline.getTimes[0];
                this.formInline.actualEndTimeTo = this.formInline.getTimes[1];
            } else {
                this.formInline.actualEndTimeFrom = '';
                this.formInline.actualEndTimeTo = '';
            }
            //
            if (!!this.formInline.dispatchTime) {
                this.formInline.dispatchTimeFrom = this.formInline.dispatchTime[0];
                this.formInline.dispatchTimeTo = this.formInline.dispatchTime[1];
            } else {
