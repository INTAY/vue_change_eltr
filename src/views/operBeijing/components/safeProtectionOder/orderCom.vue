<template>
    <div>
                    ...
                    <el-menu class="asideMeun">
                        <template v-for="(item, index) in menuMap">
                            <el-menu-item :index="index.toString()" :key="item.id" @click="pushMenuIndex(item, index)">
                                <span slot="title" class="el-icon-d-arrow-right"> &nbsp;{{ item.libName }}</span>
                            </el-menu-item>
                        </template>
                    </el-menu>
                </el-aside>
        ...
        <!-- <div class="dialog-footer">
            <el-button class="newButtonColor btnSpace listBTN" size="mini">派发</el-button>
        </div> -->
    </div>
</template>
<script>
...
import { getOrderDetail, getSubContent, getChildDetail } from '@/api/operBeijing/patrolFormManage';
