<template>
    <div>
        <el-input placeholder="请输入文字" v-model="input4" @keyup.enter.native="KeyUpEnter">
            <i slot="prefix" class="el-input__icon el-icon-search"></i>
        </el-input>
        <el-tree
            :key="treeKey"
            style="background-color:#FAFAFA;padding-left:10px;border-top: #1098C0 2px solid;"
            :props="defaultProps"
            node-key="id"
            highlight-current
            accordion
        ...
        >
            <!-- <span slot-scope="{ node, data }" style="display: inline-block;width: 95%; position: relative;" @mouseenter="mouseenter(data)" @mouseleave="mouseleaves(data)">
                <span>
...
export default {
    ...
    methods: {
        ...
        loadNode(node = null, resolve = null) {
            if (node.level === 0) {
                return resolve([{ id: '-1', name: '知识库类别管理', type: 0 }]);
            }
            var parentId = '-1';
